

import 'package:anka_intelligence_app/models/announcement_model.dart';

List<AnnouncementModel> announcementList = <AnnouncementModel>[
  AnnouncementModel(
      title: "20 Şubat Kampüs Buluşması Hk.",
      text: """Sevgili İstanbul Kodluyor’lu, 
  
Heyecanla beklenen Son Kampüs Buluşması için geri sayıma başladık. Seninle tanışmak için sabırsızlanıyoruz.

20 Şubat saat 12.00-15.30 arasında Mustafa Saffet Kültür Merkezi konferans salonunda buluşuyoruz.



Buluşmada aşağıdaki başlıkların üzerinden geçeceğiz;

-       Program bilgilendirmesi

o  Mesleki Gelişim Eğitimi’nde sizleri neler bekliyor

o  Süreç akışı

-       Dr. Ecmel Ayral ile sohbet

-        “Mesleki Gelişim Eğitimleri” detayları ve soru cevap oturumu

-       Sosyalleşme ve tanışma



Mustafa Saffet Kültür Merkezi’ne ulaşım:

Toplu taşıma ile gelmek isteyen kişiler Kadıköy-Sabiha Gökçen (M4) hattını kullanabilir. Göztepe durağında (Göztepe Köprüsü) inerek buradan 7-9 dakika yürüyerek veya otobüse binerek (2 durak) ulaşabilirler.

Kadıköyden direkt buraya gelen otobüsler; 20K, 20E, 10E. Bu otobüsler ile Örnek Mahallesi durağında inip, yolun karşısına geçerek Kültür Merkezine ulaşabilirsiniz.



Girişte “İstanbul Kodluyor Kampüs Buluşmasına” geldiğinizi söylemeniz yeterli olacaktır. Girişte hemen solda salona ulaşabileceksiniz. Sizi karşılayan gönüllü bir ekip olacak.



Açık Adres: Örnek, Şht. Cahar Dudayev Cd. No:64, 34704 Ataşehir/İstanbul



Not: Gelen kişilerin katıldığına dair QR ile yoklaması alınacaktır. Bu buluşmaya katılmayan kişilerin mesleki eğitimlere ataması yapılamamaktadır.



Buluşmaya cesaretin, özgüvenin ve motivasyonunu yanına alıp gelmen yeterli, bu buluşmada başka bir şeye ihtiyacın olmayacak.



Görüşmek üzere,

Sevgiler,



Tobeto | İstanbul Kodluyor Ekibi"""),
  AnnouncementModel(title: "Mindset Anketi", text: """
Sevgili İstanbul Kodluyorlu,



Projemize akademik bakış açılarını da eklemek için aşağıdaki formu doldurmanı rica ederiz.



Form: https://form.jotform.com/232776381646062



Not: Form sadece 1 kez doldurulabilmektedir. Önceden dolduranların tekrar doldurmasına gerek yoktur.



Sevgiler,
"""),
  AnnouncementModel(title: "Ocak Ayı Tercih Formu Bilgilendirmesi", text: """
Tercih formunu bekleyen adaylarımızın discorddaki duyuruyu takip etmesini rica ederiz.



Formu ulaşanların bugün saat 18.00'e kadar tercih formunu göndermeleri gerekmektedir.



Form ulaşmayan kişiler discord üzerinden gerekli bilgilendirmeyi ekibe ulaştırabilir.



Sevgiler,
"""),
  AnnouncementModel(title: "11 Ocak Kampüs Buluşması", text: """
Herkes için Kodlama eğitimini bitiren kişilerin katılabileceği kampüs buluşmamız 11 Ocak 2024 tarihindedir. Discorddan form paylaşılmıştır. Bu katılım formunu doldurmayan arkadaşların doldurması önemlidir.



Not: Henüz eğitime hiç başlamamış adayların buluşması, eğitimlerini bitirdikten sonra 20 Şubat 2024 tarihinde yapılacaktır.



Sevgiler,
"""),
  AnnouncementModel(title: "30 Ocak Online Hoşgeldin Buluşması-5", text: """
Sevgili İstanbul Kodluyor'lu,

30 Ocak salı günü saat 11.00'de aramıza yeni katılan adaylar ile online olarak bir araya geleceğimiz için heyecanlıyız.



İlk etapta tüm süreci birlikte konuşup, neler yapmanız gerektiğini adım adım anlatacağız. "Eğitimlerim" bölümündeki size tanımlanmış videoları izleyebilirsiniz. "Eğitime Git" butonuna tıklayarak online oturumların saatini ve içeriğini görebilirsiniz. Online oturumlara saati geldiğinde ilgili yerden tıklayarak katılabilirsiniz. Ayrıca bir mail gönderilmeyecektir. Hoşgeldin Buluşması-5 etkinliğine katılımınızı bekliyoruz. Platformu takip etmek adayların sorumluluğundadır.



Not: Eğer "Eğitimlerim" bölümünde size atanmış Hoşgeldin Buluşması- 5 başlığını görmüyorsanız, 29 Ocak pazartesiye kadar beklemenizi rica ederiz. 29 Ocak günü hala göremiyorsanız istanbulkodluyor@tobeto.com adresine mail atabilirsiniz.



Sevgiler

TOBETO | İstanbulKodluyor
""")
];
